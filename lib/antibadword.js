...
// Default badwords (fallback jika belum ada custom)
...
// Fungsi deteksi kata kasar (untuk digunakan di message handler)
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    try {
        ...
        // Bersihkan pesan untuk pengecekan
        ...
        // Hapus pesan jika autoDelete aktif
...
async function handleWarnAction(sock, chatId, senderId, foundWord, config) {
    ...
    if (warningCount >= config.maxWarnings) {
        // Kick user setelah mencapai max warnings
        try {
...
async function handleMuteAction(sock, chatId, senderId, foundWord, config) {
    try {
        const muteDuration = config.muteDuration * 60; // Convert to seconds

        // Note: WhatsApp API mungkin tidak mendukung mute langsung
        // Alternatif: Kirim peringatan dan catat di database
        await sock.sendMessage(chatId, {
            text: 'USER DIBISUKAN!\n\n' +
        ...
        // Catat di warning system sebagai alternatif
