...
function loadAntibadwordConfig(groupId) {
    try {
        ...
        console.error('Hmm, error waktu loading config antibadword nih:', error.message);
        return {};
    }
}
...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*SETTINGAN ANTIBADWORD*\n\n*.antibadword on*\nNyalain fitur antibadword\n\n*.antibadword set <action>*\nAtur aksi: delete/kick/warn\n\n*.antibadword off*\nMatiin antibadword di grup ini`
        });
    }
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: 'Wah, antibadword udah aktif dari tadi lho di grup ini~' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: 'Yeayy! Antibadword udah aku nyalain. Mau atur aksinya? Coba .antibadword set <action>' });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: 'Antibadword udah dimatiin kok~ Ga perlu dimatiin lagi deh.' });
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: 'Oke deh, antibadword udah aku matiin untuk grup ini.' });
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: 'Wah, aksinya ga sesuai nih. Pilih yang bener ya: delete, kick, atau warn~' });
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `Hore! Aksi antibadword udah diatur jadi: ${action}` });
    }

    return sock.sendMessage(chatId, { text: 'Aduh, commandnya ga kebaca nih. Coba ketik .antibadword aja buat lihat cara pakainya~' });
}
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (!antiBadwordConfig?.enabled) {
        console.log('Antibadword lagi mati nih di grup ini');
        return;
    }
    ...
    // console.log('Wah, ada kata-kata kasar nih:', userMessage);
    ...
    if (!bot?.admin) {
        // console.log('Aduh, aku bukan admin jadi ga bisa ngapa-ngapain');
        return;
    }
    ...
    if (participant?.admin) {
        //console.log('Yang ngirim admin nih, aku skip dulu ya~');
        return;
    }
    ...
    try {
        ...
        console.error('Gagal hapus pesan nih:', err);
        return;
    }
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `Hai @${senderId.split('@')[0]}, kata-kata kasar ga boleh dipakai di sini ya~`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `Maaf ya @${senderId.split('@')[0]}, kamu di-kick karena pakai kata-kata kasar. Jangan ulangi lagi ya~`,
                    mentions: [senderId]
                });
            } catch (error) {
                console.error('Error waktu kick user:', error);
            }
            break;
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `@${senderId.split('@')[0]} udah dapat 3 warning, jadi aku kick ya. Mainnya jangan kasar-kasar~`,
                        mentions: [senderId]
                    });
                } catch (error) {
                    console.error('Error waktu kick user setelah warning:', error);
                }
            } else {
                await sock.sendMessage(chatId, {
                    text: `Hai @${senderId.split('@')[0]}, ini warning ke-${warningCount}/3 ya. Awas jangan pakai kata kasar lagi~`,
                    mentions: [senderId]
                });
            }
            break;
